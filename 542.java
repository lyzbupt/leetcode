        ...
        for(int i=0; i< matrix.length;i++){
        	for(int j =0; j<matrix[0].length;j++){
        		if(matrix[i][j]==0){
        			...
        			int upCell = (i > 0)? res[i-1][j] : range;
        			int leftCell = (j>0)? res[i][j-1] : range;
        ...
        for(int i = matrix.length - 1; i>=0;i--){
        	for(int j = matrix[0].length - 1; j>=0;j--){
        		if(matrix[i][j] == 0){
        			...
        			int downCell = (i < matrix.length -1)? res[i+1][j]:range;
        			int rightCell = (j < matrix[0].length -1)? res[i][j+1]:range;
